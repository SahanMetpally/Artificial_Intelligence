...
def min_step(word, alpha, beta): # return min weight of future outcome
    if word in words_list:
        return -1
    results = []
    for next_w, m in next_word(word):
        ms = mid_step(next_w, alpha, beta)
        results.append(ms)
        if ms < beta: 
            beta = ms
        if alpha >= beta:
...
def mid_step(word, alpha, beta): # return mid weight of future outcome
    if word in words_list:
        return 0
    results = []
    for next_w, m in next_word(word):
...
def max_move(word, alpha, beta): # return max move location, play X 
    values = []
    nextWords = next_word(word)
    for next_w, m in nextWords:
